...
mainBot.start((ctx) => { 
    ...
    ctx.reply(`🌟 *MASTER CONTROL*`, { 
        ...
        reply_markup: { 
            inline_keyboard: [ 
                [{ text: '🛍️ Add Product', callback_data: 'menu_add_product' }], 
...
async function processDeposit(id, action) { 
    ...
    if (dep && dep.status === 'PENDING') { 
        if (action === 'APPROVE') { 
            ...
            return { success: true, msg: `Approved: ৳${dep.amountBdt}` }; 
        } else { 
...
app.get('/api/notices', async (req, res) => {
    res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate'); // Fixed caching bug
...
app.get('/api/photo/:fileId', async (req, res) => { 
    try { 
        const link = await mainBot.telegram.getFileLink(req.params.fileId); 
        res.redirect(link.href); 
    } catch(e) { res.status(404).send('Not found'); } 
});

// 🔥 DUAL ADMIN LOGIN & STORE CONFIG
app.get('/api/store-config', async (req, res) => {
    let conf = await prisma.storeConfig.findUnique({ where: { id: 1 } });
    if (!conf) {
        conf = await prisma.storeConfig.create({ data: { id: 1 } });
    }
...
app.post('/api/admin/login', async (req, res) => { 
    ...
    const admin = await prisma.systemAdmin.findFirst({ where: { password: password } });
    if (admin) {
    ...
    if (req.body.action === 'add') {
        ...
        try { 
            await prisma.systemAdmin.create({ 
                data: { name, email, phone, location, password: adminPass } 
            }); 
        ...
        await prisma.systemAdmin.delete({ where: { id: parseInt(req.body.id) } }); 
        res.json({success: true});
    }
});
...
app.post('/api/admin/store-config', async (req, res) => {
    ...
    await prisma.storeConfig.upsert({ 
        where: { id: 1 }, 
        update: { ownerName, ownerPhone, ownerBio, fbLink, tgLink }, 
        create: { id: 1, ownerName, ownerPhone, ownerBio, fbLink, tgLink } 
    });
    res.json({ success: true });
});
...
app.post('/api/auth/send-otp', async (req, res) => {
    ...
    await prisma.user.update({ 
        where: { id: user.id }, 
        data: { resetCode: otp, resetToken: token, resetExpiry: new Date(Date.now() + 15 * 60000) } 
    });
    ...
    const mailOptions = { 
        ...
        to: email, 
        subject: 'Security: Password Reset Code', 
        html: `${emailHeader}
            <p style="font-size: 16px; margin-bottom: 20px;">Hello ${user.firstName || ''},</p>
            <p>You requested to reset your password. Please use the 6-digit OTP code below to verify your request.</p>
            <div style="text-align:center; margin: 40px 0; background: rgba(0,0,0,0.3); padding: 20px; border-radius: 12px; border: 1px dashed #3b82f6;">
                <p style="color: #94a3b8; font-size: 12px; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 10px;">Verification Code</p>
                <h1 style="font-size:48px; letter-spacing:15px; color:#10b981; margin: 0;">${otp}</h1>
            </div>
            <p style="text-align: center; color: #94a3b8;">Or click below to reset directly:</p>
            <div style="text-align:center; margin: 20px 0;">
                <a href="${resetLink}" style="display: inline-block; background: #3b82f6; color: #ffffff; padding: 15px 35px; text-decoration: none; border-radius: 8px; font-weight: 900; letter-spacing: 1px;">DIRECT RESET LINK</a>
            </div>
            <p style="color:#ef4444; font-size:12px; text-align: center; margin-top: 30px;">⚠️ Do not share this code. It expires in 15 minutes.</p>
        ${emailFooter}` 
    };
    ...
    if (process.env.EMAIL_USER && process.env.EMAIL_PASS) { 
        ...
        res.json({ success: false, error: 'Email server offline.'}); 
    }
});
...
app.post('/api/auth/send-profile-otp', async (req, res) => {
    ...
    await prisma.user.update({ 
        where: { id: user.id }, 
        data: { resetCode: otp, resetExpiry: new Date(Date.now() + 15 * 60000) } 
    });
    
    const mailOptions = { 
        from: `"AURA SECURITY" <${process.env.EMAIL_USER}>`, 
        to: user.email, 
        subject: 'Profile Update Security Code', 
        html: `${emailHeader}
            <h2 style="color: #facc15;">Security Alert</h2>
            <p>An attempt was made to change your account password. Use this OTP to verify.</p>
            <h1 style="text-align:center; font-size:40px; letter-spacing:10px; color:#3b82f6;">${otp}</h1>
        ${emailFooter}` 
    };
...
app.post('/api/auth/reset', async (req, res) => {
    ...
    if (token) {
        ...
        user = await prisma.user.findFirst({ where: { email: email, resetCode: code } });
    }
    
    if (!user || !user.resetExpiry || user.resetExpiry < new Date()) {
        return res.json({ success: false, error: 'Invalid or expired code/token.' });
    }
    
    await prisma.user.update({ 
        where: { id: user.id }, 
        data: { password: newPassword, resetCode: null, resetToken: null, resetExpiry: null } 
    });
...
app.get('/reset-password', (req, res) => {
    ...
    <body class="bg-slate-950 flex justify-center items-center h-screen font-sans">
        <div class="bg-slate-900/80 p-10 rounded-3xl w-full max-w-md border border-slate-800 shadow-[0_0_50px_rgba(59,130,246,0.2)] text-center backdrop-blur-xl">
            <div class="w-16 h-16 bg-blue-600 rounded-full mx-auto flex items-center justify-center text-white text-2xl mb-6 shadow-[0_0_20px_rgba(59,130,246,0.5)]">🔒</div>
            <h2 class="text-3xl font-black text-white mb-2">New Password</h2>
            <p class="text-slate-400 text-sm mb-8">Secure your AURA STORE account.</p>
            <input type="password" id="pass" placeholder="Enter new password" class="w-full bg-slate-950 border border-slate-700 text-white px-5 py-4 rounded-xl font-bold mb-6 outline-none focus:border-blue-500 transition-colors">
            <button onclick="savePass()" class="w-full bg-blue-600 text-white font-black py-4 rounded-xl hover:bg-blue-500 transition-transform active:scale-95 shadow-lg uppercase tracking-widest">Confirm & Login</button>
        </div>
        <script>
            async function savePass(){ 
                ...
                if(data.success){ 
                    Swal.fire({title:'Success!', text:'Password Updated.', icon:'success', background:'#0f172a', color:'#fff', confirmButtonColor:'#3b82f6'})
                    .then(()=>window.location.href='/login'); 
                } else { 
                    Swal.fire({title:'Error', text:data.error, icon:'error', background:'#0f172a', color:'#fff'}); 
                } 
            }
        </script>
    </body>
...
app.post('/api/user/update', async (req, res) => { 
    ...
    try { 
        ...
        // Password Change Logic Requires OTP
        if (password) {
...
app.post('/api/feedback', async (req, res) => { 
    try { 
        ...
        if (ADMIN_ID) { 
            ...
            feedbackBot.telegram.sendMessage(ADMIN_ID, `📢 *FEEDBACK*\n👤 ${u.firstName}\n📌 ${subject}\n📝 ${message}`, { parse_mode: 'Markdown' }).catch(()=>{}); 
        } 
    } catch(e){} 
});
...
app.post('/api/register', async (req, res) => { 
    try { 
        ...
        await prisma.user.create({ data: { firstName: name, email, password, refCode: generateRefCode(), isVerified: true } }); 
...
app.post('/api/login', async (req, res) => { 
    try { 
        ...
        if (user && user.password === req.body.password) { 
            ...
            res.json({ success: true, user: { id: user.id, name: user.firstName, email: user.email, balanceBdt: user.balanceBdt, role: user.role, avatar: user.avatar, loyaltyPoints: user.loyaltyPoints } }); 
        } else { 
...
app.post('/api/chat', async (req, res) => { 
    ...
    } catch (error) { res.json({ reply: "Server error." }); } 
});

// Admin Panel Fetch APIs
...
app.post('/api/admin/user/action', async (req, res) => { 
    ...
    } else if(req.body.action === 'role') { 
        await prisma.user.update({where:{id:parseInt(req.body.id)}, data:{role:req.body.role}}); 
    } 
    res.json({success:true}); 
});
