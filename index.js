...
const addProductWizard = new Scenes.WizardScene('ADD_PRODUCT_SCENE',
  ...
  async (ctx) => { 
      if (ctx.message.text === '/finish') {
          ...
          try { 
              await prisma.product.create({ data: { name: name || "Premium Item", category: category || "Premium", price: price || 0, abilities: abilities || "Best quality product.", stock: stock || 1, sizes: sizes || [], colors: colors || [], imageIds: imageIds } }); 
...
// --- 🔥 RIDER APIs UPDATED (Shows Pending & Shipped Orders) ---
...
// 🔥 RIDER ORDER FETCH LOGIC UPDATED TO SHOW ALL ACTIVE ORDERS
app.post('/api/rider/orders', async (req, res) => { 
    try { 
        ...
        const orders = await prisma.purchase.findMany({ 
            where: { status: { in: ['PENDING', 'RECEIVED', 'SHIPPED'] } }, 
