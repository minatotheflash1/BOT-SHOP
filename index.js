...
// ================= TELEGRAM WIZARDS =================
const addProductWizard = new Scenes.WizardScene('ADD_PRODUCT_SCENE', (ctx) => { ctx.reply('🛍️ 1. Product Name?'); return ctx.wizard.next(); }, (ctx) => { ctx.wizard.state.name = ctx.message.text; ctx.reply('🗂️ 2. Category?'); return ctx.wizard.next(); }, (ctx) => { ctx.wizard.state.category = ctx.message.text; ctx.reply('💵 3. Price in BDT (৳)?'); return ctx.wizard.next(); }, (ctx) => { ctx.wizard.state.price = parseFloat(ctx.message.text) || 0; ctx.reply('🪄 4. Description?'); return ctx.wizard.next(); }, (ctx) => { ctx.wizard.state.abilities = ctx.message.text; ctx.reply('📦 5. Stock Quantity?'); return ctx.wizard.next(); }, (ctx) => { ctx.wizard.state.stock = parseInt(ctx.message.text) || 1; ctx.reply('📏 6. Sizes?'); return ctx.wizard.next(); }, (ctx) => { ctx.wizard.state.sizes = ctx.message.text.toLowerCase() === 'none' ? [] : ctx.message.text.split(',').map(s=>s.trim()); ctx.reply('🎨 7. Colors?'); return ctx.wizard.next(); }, (ctx) => { ctx.wizard.state.colors = ctx.message.text.toLowerCase() === 'none' ? [] : ctx.message.text.split(',').map(s=>s.trim()); ctx.wizard.state.imageIds = []; ctx.reply('📸 8. Send Photos one by one.\n✅ Type /finish when done.'); return ctx.wizard.next(); }, async (ctx) => { if (ctx.message.text === '/finish') { if (ctx.wizard.state.imageIds.length === 0) { ctx.reply('❌ Send at least 1 photo!'); return; } const { name, category, price, abilities, stock, sizes, colors, imageIds } = ctx.wizard.state; try { await prisma.product.create({ data: { name: name || "Premium Item", category: category || "Premium", price: price || 0, abilities: abilities || "Best quality product.", stock: stock || 1, sizes: sizes || [], colors: colors || [], imageIds: imageIds } }); ctx.reply(`🎉 *Product Added!*`, { parse_mode: 'Markdown' }); } catch(e) { ctx.reply(`❌ DB Error: ${e.message}`); } return ctx.scene.leave(); } if (ctx.message.photo) { ctx.wizard.state.imageIds.push(ctx.message.photo[ctx.message.photo.length - 1].file_id); ctx.reply(`🖼️ Photo received! (${ctx.wizard.state.imageIds.length} total). Send another or /finish`); return; } } );
...
mainBot.action('menu_add_product', (ctx) => { ctx.answerCbQuery(); ctx.scene.enter('ADD_PRODUCT_SCENE'); }); mainBot.action('menu_add_notice', (ctx) => { ctx.answerCbQuery(); ctx.scene.enter('ADD_NOTICE_SCENE'); }); mainBot.action('menu_flash_sale', async (ctx) => { let fs = await prisma.flashSale.findFirst(); if(fs && fs.isActive) { await prisma.flashSale.update({ where: { id: fs.id }, data: { isActive: false } }); ctx.answerCbQuery('Flash Sale Stopped!'); ctx.reply('🛑 Flash Sale OFF.'); } else { ctx.answerCbQuery(); ctx.scene.enter('FLASH_SALE_SCENE'); } }); mainBot.action('menu_clear_notices', async (ctx) => { await prisma.notice.deleteMany({}); ctx.answerCbQuery('🗑️ Notices deleted!'); ctx.reply('✅ *Notices cleared.*', { parse_mode: 'Markdown' }); }); mainBot.action('toggle_maintenance', async (ctx) => { isMaintenance = !isMaintenance; const mStatus = isMaintenance ? '🔴 ON' : '🟢 OFF'; await ctx.editMessageReplyMarkup({ inline_keyboard: [ [{ text: '🛍️ Add Product', callback_data: 'menu_add_product' }, { text: '⚡ Flash Sale', callback_data: 'menu_flash_sale' }], [{ text: `🛠️ Maintenance Mode: ${mStatus}`, callback_data: 'toggle_maintenance' }], [{ text: '📢 Add Notice', callback_data: 'menu_add_notice' }, { text: '🗑️ Clear Notices', callback_data: 'menu_clear_notices' }], [{ text: '📊 View Store Stats', callback_data: 'menu_stats' }] ] }).catch(e=>{}); ctx.answerCbQuery(`Maintenance ${isMaintenance ? 'ON' : 'OFF'}`); }); mainBot.action('menu_stats', async (ctx) => { ctx.answerCbQuery('Loading Stats...'); const users = await prisma.user.count(); const orders = await prisma.purchase.count(); const revAggr = await prisma.purchase.aggregate({ _sum: { advancePaid: true } }); ctx.reply(`📊 *LIVE STATS*\n👥 Users: ${users}\n📦 Orders: ${orders}\n💰 Advance: ৳${revAggr._sum.advancePaid || 0}`, { parse_mode: 'Markdown' }); });
...
// 🔥 RIDER WITHDRAW TELEGRAM BOT ACTIONS
...
logBot.action(/rw_rej_(.+)/, async (ctx) => {
    ...
    if(rw && rw.status === 'PENDING') {
        ...
        await prisma.rider.update({ where: { id: rw.riderId }, data: { walletBalance: { increment: rw.amount } } }); // Refund
...
// ================= EXPRESS APIs =================
app.post('/api/chat', async (req, res) => { try { const response = await fetch('https://api.deepseek.com/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${process.env.DEEPSEEK_API_KEY}` }, body: JSON.stringify({ model: 'deepseek-chat', messages: [{ role: 'system', content: `You are a friendly Customer Support AI for AURA STORE.` }, { role: 'user', content: req.body.message }] }) }); const data = await response.json(); res.json({ reply: data.choices[0].message.content }); } catch (error) { res.json({ reply: "Server error." }); } });
...
app.get('/api/verify-email/:token', async (req, res) => { try { const user = await prisma.user.findFirst({ where: { verifyToken: req.params.token } }); if (!user) return res.status(400).send('<body style="background:#050b14;color:white;font-family:sans-serif;text-align:center;padding-top:20%;"><h2 style="color:#ef4444;">❌ Invalid or Expired Token!</h2></body>'); await prisma.user.update({ where: { id: user.id }, data: { isVerified: true, verifyToken: null } }); res.send('<body style="background:#050b14;color:white;font-family:sans-serif;text-align:center;padding-top:20%;"><h2 style="color:#10b981;">✅ Email Verified Successfully!</h2><p>Redirecting to login...</p><script>setTimeout(()=>window.location.href="/login", 2000);</script></body>'); } catch(e) { res.status(500).send('Error verifying email.'); } });
...
// 🔥 DELIVER ORDER & ADD 50 TK COMMISSION
app.post('/api/rider/order/action', async (req, res) => { 
    try { 
        ...
        // 💰 50 Taka Commission Added to Rider Wallet Here
        await prisma.rider.update({ where: { id: rider.id }, data: { deliveryCount: { increment: 1 }, walletBalance: { increment: 50.0 }, totalEarned: { increment: 50.0 } } }); 
        
        if(ADMIN_ID) { const msg = `✅ *ORDER DELIVERED*\n\n📦 *Order ID:* #${purchase.id}\n🛒 *Product:* ${purchase.product.name}\n👤 *Customer:* ${purchase.user.firstName}\n🚴 *Delivered By:* ${rider.name} (${rider.phone})\n💰 *Cash Collected:* ৳${purchase.dueCod}`; logBot.telegram.sendMessage(ADMIN_ID, msg, { parse_mode: 'Markdown' }).catch(e=>{}); } 
        res.json({ success: true }); 
    } catch(e) { res.json({ success: false }); } 
});

// 🔥 NEW: RIDER WITHDRAW API
app.post('/api/rider/withdraw', async (req, res) => {
    const { riderId, amount, method, number } = req.body;
    try {
        const rider = await prisma.rider.findUnique({ where: { id: parseInt(riderId) } });
        if (!rider || rider.walletBalance < amount || amount < 50) return res.json({ success: false, error: 'Invalid amount or insufficient balance.' });

        // Deduct from wallet immediately
        await prisma.rider.update({ where: { id: rider.id }, data: { walletBalance: { decrement: parseFloat(amount) } } });

        const rw = await prisma.riderWithdraw.create({ data: { riderId: rider.id, amount: parseFloat(amount), method, number } });

        if (ADMIN_ID) {
            const msg = `💸 *RIDER WITHDRAW REQUEST*\n\n🚴 Rider: ${rider.name}\n💰 Amount: ৳${amount}\n🏦 Method: ${method.toUpperCase()}\n📱 Number: \`${number}\``;
            logBot.telegram.sendMessage(ADMIN_ID, msg, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '✅ Approve', callback_data: `rw_app_${rw.id}` }, { text: '❌ Reject', callback_data: `rw_rej_${rw.id}` }]] } }).catch(e => {});
        }
        res.json({ success: true });
...
app.get('/api/admin/stats', async (req, res) => { 
    ...
    // Fetch Rider Withdrawals for Admin Panel
    const riderWithdraws = await prisma.riderWithdraw.findMany({ include: { rider: true }, orderBy: { createdAt: 'desc' } });
    
    res.json({ users: await prisma.user.count(), deposits: await prisma.deposit.findMany({ include: { user: true }, take: 20, orderBy: { createdAt: 'desc' } }), products: await prisma.product.findMany(), userList: await prisma.user.findMany({ take: 50, orderBy: { createdAt: 'desc' } }), riderList: await prisma.rider.findMany({ orderBy: { createdAt: 'desc' } }), orders: recentPurchases, revenue, riderWithdraws }); 
});
...
// 🔥 RIDER WITHDRAW ADMIN WEB ACTION
app.post('/api/admin/rider-withdraw/action', async (req, res) => {
    if (req.body.password !== (process.env.ADMIN_PASSWORD || 'Ananto01@$')) return res.status(403).json({ error: 'Unauthorized' });
    const { id, action } = req.body;
    const rw = await prisma.riderWithdraw.findUnique({ where: { id: parseInt(id) } });
    if (rw && rw.status === 'PENDING') {
        if (action === 'APPROVE') { await prisma.riderWithdraw.update({ where: { id: rw.id }, data: { status: 'APPROVED' } }); } 
        else { await prisma.riderWithdraw.update({ where: { id: rw.id }, data: { status: 'REJECTED' } }); await prisma.rider.update({ where: { id: rw.riderId }, data: { walletBalance: { increment: rw.amount } } }); }
